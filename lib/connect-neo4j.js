module.exports = function (session) {
  ...
  class Neo4jStore extends Store {
    constructor(options = {}) {
      ...
      this.isDisposable = options.isDisposable || (() => true);
      this.noDisposeOnSet = options.noDisposeOnSet || true
      this.dbName = options.dbName || "neo4j"
    ...
    get(sid, cb = noop) {
      ...
      let sess = this.client.session({database:this.dbName})
      sess
        ...
        .then((result) => {
          ...
          if (expires && expires < Date.now() && this.isDisposable(sid)) {
    ...
    set(sid, session, cb = noop) {
      ...
      if (!this.disableTTL) {
        ...
        if (this.isDisposable(sid) && ttl <= 0) {
    ...
    startInterval() {
      var self = this
      var ms = this.checkPeriod
      if (ms && typeof ms === 'number') {
        clearInterval(this._checkInterval)
        this._checkInterval = setInterval(function () {
          self.prune() // iterates over the entire cache proactively pruning old entries
        }, Math.floor(ms)).unref()
      }
    }
